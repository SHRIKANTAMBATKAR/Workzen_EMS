...
export default function Navbar({ user }) {
    return (
        <header className="h-24 bg-white/5 backdrop-blur-xl border-b border-white/5 px-10 flex items-center justify-between sticky top-0 z-20">
            <div className="flex items-center gap-4 bg-white/5 px-6 py-3 rounded-2xl w-[450px] border border-white/5 focus-within:border-accent transition-all">
                <Search size={20} className="text-slate-500" />
                <input
                    type="text"
                    placeholder="Search anything..."
                    className="bg-transparent outline-none text-sm w-full text-white placeholder:text-slate-500 font-medium"
                />
            </div>
